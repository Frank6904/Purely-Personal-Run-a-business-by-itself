...
4. Voice Lock (hardcoded rules + live brain check — replaces Voice DNA Check)
5. Intake Form
...
This six-file manifest is CANONICAL — identical to the Tailor SKILL.md's
Injection Point 3 and its zip structure. Do not add, drop, or substitute filenames.

```markdown
Read ALL reference files before writing:
- `/references/voice-dna.md` — [PARTICIPANT]'s voice rules, banned words, hook archetypes (REGENERATED from their Voice DNA / brain §5 — never Daniel Paul's copy)
...
- `/references/scrub.md` — 9-scrub output refinement system, full AI pattern blacklist
- `/references/[participant]-context.md` — ICP, offer, proof points, competitors, output rules
- PLUS every skill-specific reference the original skill ships (methodology files like
  email-craft.md or chat-selling.md or creator-systems.md, craft files, visual-standards.md).
  Carry them all forward, rewritten only where they hold participant context. A fitted
  skill with fewer reference files than its original is a build failure.
```

Separate ICP or offer details do not get their own files — the Tailor consolidates
them into `[participant]-context.md` from the Tailor Briefing.
...
**Voice Lock = hardcoded rules + live brain check; brain wins on conflict.**
Every fitted skill opens its Voice Lock with a STEP 0 context check that reads
BUSINESS-BRAIN.md at runtime — so a participant who updates their brain updates
all fitted skills without re-tailoring. This is the single most important
architectural feature of a fitted skill.

```markdown
## VOICE LOCK

STEP 0 — CONTEXT CHECK (always first, never skipped):
If BUSINESS-BRAIN.md (or business-brain.md / brain.md) is in Project Knowledge or
attached, read it now. Its Voice DNA, ICP, offer, proof, sign-off, and design
tokens OVERRIDE the hardcoded rules below and every file in references/ wherever
they conflict. The brain wins. If no brain exists, the hardcoded rules below apply.
...
□ References folder contains the six canonical files (Section 3) plus every skill-specific reference from the original — including scrub.md, [participant]-context.md, and visual-standards.md where the original ships it
□ voice-dna.md is regenerated from the participant's Voice DNA / brain — never Daniel Paul's copy
□ Voice Lock opens with the STEP 0 brain check — hardcoded rules + live brain check, brain wins on conflict
