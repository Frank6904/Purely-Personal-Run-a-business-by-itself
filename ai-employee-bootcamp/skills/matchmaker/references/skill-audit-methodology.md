...
## THE GAP TAXONOMY — THE 7 DIMENSIONS

Use this taxonomy to classify every gap found. It is the SAME canonical taxonomy
the Matchmaker's SKILL.md audits, in the same order:
1 Voice · 2 ICP · 3 Offer & Positioning · 4 Industry & Competitor ·
5 Proof & Credibility · 6 Formatting & Output Rules · 7 Scrub

Not all gaps are equal. Rate each gap: Critical (will break the output every time)
/ Major (degrades quality significantly) / Minor (small inconsistency, fixable with
light editing). The per-dimension severity anchors live in SKILL.md Step 3 — use them.

### Dimension 1 — Voice (Critical to Major depending on extent)
...
Pull directly from the brain's §5 Voice DNA, or the Voice DNA document.

---

### Dimension 2 — ICP (Critical)
...
### Dimension 3 — Offer & Positioning (Major to Critical depending on skill)
The skill doesn't know what it's ultimately selling toward — or who is selling it.
This dimension absorbs the identity gap: no participant name, role, offer, or
market position embedded means every single output is off-brand by default.
Critical for: DM Sequence Writer, Sales Call Prep.
Major for: Caption Writer, Newsletter Writer.
Lower for: Content Strategy (content doesn't always sell directly).

**Signs:** The skill references "your offer" without knowing what the offer is.
The opening role definition uses only generic terms — no reference to the
participant's specific position, methodology, or market.

**Fix pattern:** Add a WHO block at the top of the skill that hardcodes:
name, role, market, offer name + transformation, ICP, unique angle, competitive position.

---

### Dimension 4 — Industry & Competitor (Major)
...
### Dimension 5 — Proof & Credibility (Major to Critical)
The skill has no evidence to write with — or nothing stopping it inventing evidence.

**Signs:** No proof points, results, or testimonials embedded.
Claims default to vague authority ("many clients," "proven results").

**Fix pattern:** Add a PROOF block: minimum 3 proof points, each with
result + timeframe + client type — plus the standing rule that missing proof
is OMITTED, never fabricated.

---

### Dimension 6 — Formatting & Output Rules (Minor to Major depending on platform)
...
### Dimension 7 — Scrub (Minor by default for stock starters)
The starters natively enforce Scrubs 1–3 (em dashes, banned words, AI blacklist)
via their gates. Rate only gaps in Scrubs 4–9, default severity Minor for stock
starters; Critical only if a FITTED skill lost its scrub protections.

**Signs:** Quality gate has no checks for closers, structural patterns,
specificity, tonal patterns, or formatting tells (Scrubs 4–9). Fitted skill
shipped without scrub.md or with the scrub checks stripped from its gate.

**Fix pattern:** Bundle scrub.md into the fitted skill's references and add
the Scrubs 4–9 checks to its quality gate.
...
An outreach skill with no Offer & Positioning data is critical. Same gap in a strategy skill is minor.

### Step 2 — Read the skill against the 3-layer anatomy
Work through Layer 1 (Role), Layer 2 (Knowledge Inputs), Layer 3 (Output Rules).
Document every gap you find using the 7-dimension taxonomy above.

### Step 3 — Cross-reference against the brain or all foundation documents
Run the same 7-dimension audit as for the starter skills:
1 Voice · 2 ICP · 3 Offer & Positioning · 4 Industry & Competitor ·
5 Proof & Credibility · 6 Formatting & Output Rules · 7 Scrub.
